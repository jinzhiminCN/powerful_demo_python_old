...
# shufflenet神经网络模型。
# ==============================================================================
from tensorflow_basic.shufflenet_v1_demo.shufflenet_input_data import get_data
from tensorflow_basic.shufflenet_v1_demo.shufflenet_architecture import *
...
def get_shufflenet(
    ...
    Arguments:
        initial_lr: A float number, initial learning rate.
        weight_decay: A float number, L2 regularization.
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('input_pipeline'):
            data = get_data(NUM_CLASSES, IMAGE_SIZE)

        with tf.variable_scope('inputs'):
            x_input = tf.placeholder_with_default(
                data['x_batch'], [None, IMAGE_SIZE, IMAGE_SIZE, 3], 'X'
            )
            y_input = tf.placeholder_with_default(
        ...
        logits = graph_mapping(
            x_input, is_training, NUM_CLASSES,
        ...
        with tf.variable_scope('log_loss'):
            log_loss = tf.losses.softmax_cross_entropy(y_input, logits)
        ...
        with tf.variable_scope('utilities'):
            ...
            is_equal = tf.equal(tf.argmax(predictions, 1), tf.argmax(y_input, 1))
