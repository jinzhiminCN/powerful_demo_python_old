...
def get_data(num_classes, image_size):
    ...
    train_dataset = train_dataset.map(
        lambda x: parse_and_preprocess(x, image_size, augmentation=True),
    ...
    val_dataset = val_dataset.map(
        lambda x: parse_and_preprocess(x, image_size)
    )
...
def parse_and_preprocess(example_proto, image_size, augmentation=False):
    """
    TFRecord元素解析和预处理。
    :param example_proto: protobuf元素
    :param image_size: 图像尺寸
    :param augmentation: 是否进行增强
    :return:
    """
    features = {
